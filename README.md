...
- ⏲️ **Duration Parsing** - Convert between natural language durations and TimeSpan with calendar unit support - see [Integration Guide](INTEGRATION.md#duration-parsing-bidirectional) for full API
- 🛡️ **Minimum Interval Floor** - Reject schedules that fire more often than an allowed floor via `ScheduleParserOptions.MinInterval` - see [Integration Guide](INTEGRATION.md#rejecting-schedules-that-fire-too-often-mininterval) for full API
...
[Integration Guide](INTEGRATION.md#duration-parsing-bidirectional) for the
...
[Integration Guide](INTEGRATION.md#rejecting-schedules-that-fire-too-often-mininterval)
for details.
...
- **[Architecture](ARCHITECTURE.md)** - Design philosophy and implementation details
- **[DSL Specification](DSL-SPECIFICATION.md)** - Complete grammar and validation rules
- **[Integration Guide](INTEGRATION.md)** - Usage patterns, timezone handling, testing
